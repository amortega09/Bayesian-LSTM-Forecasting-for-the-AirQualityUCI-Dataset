import pandas as pd
import numpy as np
import matplotlib.pyplot as plt # Still useful for local plotting/saving images
...
import os # Added for path checking and saving
import joblib # Added for saving/loading scalers
...
# NOTE: Fit RFE on X, then transform. Keep X_rfe as a DataFrame for feature names later.
rfe.fit(X, target)
X_rfe = X[X.columns[rfe.support_]]


selected_features = X_rfe.columns.tolist() # Update selected_features based on X_rfe
...
X_scaled = xscaler.fit_transform(X_rfe) # Use X_rfe for scaling
...
XSCALER_PATH = "xscaler.pkl"
YSCALER_PATH = "yscaler.pkl"
RFE_PATH = "rfe.pkl" # Save the RFE object as well, it contains selected features/ranking

print(f"\n--- Saving trained model parameters to {MODEL_PARAMS_PATH} ---")
pyro.param.get_param_store().save(MODEL_PARAMS_PATH)

print(f"--- Saving scalers to {XSCALER_PATH} and {YSCALER_PATH} ---")
joblib.dump(xscaler, XSCALER_PATH)
joblib.dump(yscaler, YSCALER_PATH)

print(f"--- Saving RFE object to {RFE_PATH} ---")
joblib.dump(rfe, RFE_PATH) # Save the RFE object
...
def predict_with_variational_inference(model, x_data, n_samples=100):
    ...
    # else: # Removed warning for cleaner output, only if y_pred is not 1D
...
# Use the already fitted RFE object from earlier, and X from before scaling
# X at this point is after differencing and rolling mean, with cyclic features, but before RFE transform
rf_estimator_for_importance = RandomForestRegressor(n_estimators=100, random_state=42)
rf_estimator_for_importance.fit(X, target) # Fit on the full feature set (before RFE)
...
plt.subplot(3, 3, 6)
# Use the already calculated features_plot and importances_plot from earlier
plt.barh(range(len(features_plot)), importances_plot)
...
print(f"Model RMSE: {rmse_bayesian:.4f}")
